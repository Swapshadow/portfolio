...
const clean = (v='') => v.replace(/<!\[CDATA\[|\]\]>/g,'').replace(/<[^>]*>/g,' ').replace(/&nbsp;/gi,' ').replace(/\s+/g,' ').trim();
const pick=(block,tags)=>{for(const t of tags){const m=block.match(new RegExp(`<${t}[^>]*>([\\s\\S]*?)<\\/${t}>`,'i'));if(m)return clean(m[1]);const m2=block.match(new RegExp(`<${t}[^>]*href="([^"]+)"[^>]*/?>`,'i'));if(m2)return clean(m2[1]);}return '';};
...
for (const s of RSS_SOURCES){
  try{
    ...
    for(const b of blocks){
      const title=pick(b,['title']); const url=normalizeUrl(pick(b,['link','id'])); if(!title||!url) continue;
      const summary=pick(b,['description','summary','content','content:encoded']).slice(0,280);
      const dRaw=pick(b,['pubDate','published','updated','dc:date']);
      const d= new Date(dRaw); const publishedAt = Number.isNaN(d.getTime())?generatedAt:d.toISOString();
      const ctx=`${title} ${summary}`;
      merged.push({id:crypto.createHash('sha1').update(`${url}|${s.name}`).digest('hex'),title,url,source:s.name,publishedAt,summary,type:type(ctx,s.defaultType),severity:sev(ctx)});
      count++;
    }
...
const unique=new Map(); for(const i of merged){const k=normalizeUrl(i.url)||`${i.title.toLowerCase()}|${i.source.toLowerCase()}`; if(!unique.has(k)) unique.set(k,i);} 
